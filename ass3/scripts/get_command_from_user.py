...
def distance_to_colored_object_helper(color) :
	...
	bridge = CvBridge()
        ...
        if (color == 'blue'): 
            lower = np.array([170,140,90], dtype="uint8")
            upper = np.array([255,170,120], dtype="uint8")
        elif (color == 'red'):
...
def find_colored_object():
		...
		rospy.sleep(1)      
                ...
                print "hello"
		for x in range(0, 6):
			turn_around_helper(60, True) # check all field around the robot
			print " afterr turn_around_helper" 	
			distance = distance_to_colored_object_helper(color)
			while found_obj == "not initialized":
              			pass
           		if(found_obj):
